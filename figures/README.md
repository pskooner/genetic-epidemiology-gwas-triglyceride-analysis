## Figures

### Phenotype Distributions

- [Triglyceride distribution](figures/triglyceride_distribution.png)
- [BMI distribution](figures/bmi_distribution.png)
- [Log-transformed triglyceride distribution](figures/log_triglyceride_distribution.png)
- [Log-transformed BMI distribution](figures/log_bmi_distribution.png)

### Genetic Association Results

- [Chromosome 19 association plot](figures/manhattan_plot.png)
- [Q-Q plot of association p-values](figures/qq_plot.png)
- [Regional association plot](figures/regional_association_plot.png)

---

## Key Visualizations

### Chromosome 19 Association Plot

[![Chromosome 19 association plot](figures/manhattan_plot.png)](figures/manhattan_plot.png)

### Q-Q Plot

[![Q-Q plot of association p-values](figures/qq_plot.png)](figures/qq_plot.png)

### Regional Association Plot

[![Regional association plot](figures/regional_association_plot.png)](figures/regional_association_plot.png)
